...
class  App extends React.Component {
  ...
  componentDidMount(){
    axios.get("http://13.233.50.93:9999/api/")
    .then((res)=>{
      this.setState({
        users:res.data,
        id:0,
  ...
  submit(evenet,id){
    ...
    if(id===0){
      axios.post("http://13.233.50.93:9999/api/",{
      ...
      axios.put("http://13.233.50.93:9999/api/",{
        id:id,
        name:this.state.name,
        email:this.state.email,
        password:this.state.password
  ...
  delete(id){
    axios.delete("http://13.233.50.93:9999/api/"+id)
    .then(()=>{
      this.componentDidMount();
    })
  }
  edit(id){
    axios.get("http://13.233.50.93:9999/api/"+id)
    .then((res)=>{
      this.setState({
        id:res.data.id,
        name:res.data.name,
